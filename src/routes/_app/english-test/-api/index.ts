  ...
  return queryOptions({
    ...
    queryFn: async (): Promise<EnglishTestAttemptsResponse> => {
      ...
      if (
        ...
        const totalPages =
          typeof inner.totalPages === "number" && inner.totalPages > 0
            ? inner.totalPages
      ...
      if (
        ...
        const total =
          typeof resData.total === "number"
            ...
            : resData.data.length;
        const totalPages =
          typeof resData.totalPages === "number" && resData.totalPages > 0
            ? resData.totalPages
            : Math.max(1, Math.ceil(total / limit));
        return {
          data: resData.data,
  ...
  return useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({
        queryKey: ["english-test-attempts"],
      });
    },
  });
}
...
export function useGetLevelDefinitions(params?: ListParams) {
  return queryOptions({
    ...
    queryFn: async (): Promise<EnglishLevelDefinitionsResponse> => {
      const res = await api.get("/admin/english-test/level-definitions", {
        params,
      });
  ...
  return useMutation({
    mutationFn: async (data: CreateLevelDefinitionInput) => {
      const res = await api.post(
        "/admin/english-test/level-definitions",
        buildLevelPayload(data),
      );
      return res.data;
    },
    onSuccess: async () => {
      await queryClient.invalidateQueries({
        queryKey: ["english-test-levels"],
      });
    },
  });
}
...
export function useUpdateLevelDefinition() {
  ...
  return useMutation({
    mutationFn: async ({
      id,
      ...data
    }: CreateLevelDefinitionInput & { id: string }) => {
      const res = await api.put(
        `/admin/english-test/level-definitions/${id}`,
        buildLevelPayload(data),
      );
      return res.data;
    },
